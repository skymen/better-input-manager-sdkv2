export const config = {
  ...
  displayText: "Set joystick [i]{0}[/i] Y to [i]{1}[/i] for player [i]{2}[/i] with control scheme [i]{3}[/i] (prevent auto switch: [i]{4}[/i])",
...
export default function (name, y, player, scheme, preventAutoSwitch) {
  this._SetJoystickValueY(name, y, player, scheme, preventAutoSwitch);
}
